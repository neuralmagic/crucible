...
| What else is off the table? | `[workflow]` and `[preflight]` — both need scores, so a task manifest rejects them at load. `objective = "task"` is reserved on scored judges. |

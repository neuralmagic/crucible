...
| **as a plan** | Optional: `--graph-loop` runs each iteration as a work-graph plan (`propose -> apply -> measure -> decide`) through the shared executor, with the same decisions and events. | [Work graphs](./work-graphs.md) |

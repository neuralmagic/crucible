...
<!-- SIGNATURE: sha256:1a0b475a86f706cded41eb0081995d019d0f74efa679e96701bfca60fb92d966 -->
...
- plan_admitted: { plan_version, reason, budget_usd, tasks, max_asks }, emitted once after the graph is admitted and before any dispatch. Each task carries its name, kind, dependencies, session, needs, required flag, join, stage, and the workflows it may ask for. max_asks is the run's bound on asks under [RFC-0002:C-ASKS](../rfc/RFC-0002.md#rfc-0002c-asks), 0 in a lane that admits none. This is the consumer's route to the graph, so no gate label has to encode it. A consumer computing a verdict must skip tasks whose stage is epilogue, which is why the field is on the wire rather than inferable.
- asks_emitted: { task, asks }, emitted once per task that emitted any, as that task settles passing and before its task_result row. Each ask carries an emitter-supplied key, the workflow it names, and that workflow's parameter values. The key must be rejected on decode, not only on construction, so the wire is where the key rules are enforced rather than a way around them.
...
- plan_admitted carries the run's ask bound and each task's askable workflows, and asks_emitted precedes the emitting task's row

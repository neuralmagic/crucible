...
<!-- SIGNATURE: sha256:626df3cda54cdc965c0fff19c4a10865743e08badd8c74ce49a4d21d51a6a4b5 -->
...
A task declares what it needs of the workspace as workspace = "shared", "readonly", or "worktree", and an absent declaration means "shared". An author must not declare concurrency; the engine must derive it from these declarations. A shared task writes the shared workspace and must not run concurrently with any other task. A readonly task must not change the shared workspace, and may run in it concurrently with other ready readonly and worktree tasks. A worktree task must run in a disposable clone of the shared workspace as it stands, uncommitted state included, and may run concurrently with other ready readonly and worktree tasks; its workspace state must be discarded. Only a task's declared output continues through the graph: its JSON output, and any files it declared under emits_files.

The engine must hold a readonly task to its declaration. It must compare the shared workspace's content, as the workspace's version control would record it, and the branch and commit its HEAD names, before and after the tasks that ran in it concurrently; a change must fail every readonly task among them, whatever that task reported, must name the changed paths, and must be discarded before any other task is dispatched. The engine cannot tell concurrent writers in one tree apart, so it must not attribute the change to one of them. A readonly task must not declare emits_files. Readonly tasks running concurrently share one directory of staged inputs, so the engine must not run two concurrently unless the same files are staged for each. Every task's result file must be named for that task, so that tasks sharing a workspace cannot read or overwrite each other's results.

The generated TOML carries the declaration under the key isolation: absent for "shared", "readonly" or "worktree" otherwise. Run identity hashes the serialized graph, and this keeps a graph that declares no readonly task serialized exactly as it was before readonly existed. The compiler must refuse the retired isolated argument at the argument, naming the workspace values that replace it; it must not accept it as an alias, because it names a mechanism rather than what the task needs.

session = "name" must preserve one logical agent conversation across dependency-ordered tasks and loop iterations. Tasks sharing a session must not run in a worktree and must be dependency-ordered.
...
- isolated is replaced by workspace = shared | readonly | worktree, from which the engine derives concurrency; a readonly task that changes the workspace fails and the change is discarded; each task writes its own result file

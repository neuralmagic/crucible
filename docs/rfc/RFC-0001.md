...
<!-- SIGNATURE: sha256:fe59b6dc52c024ecdedfbb0bc8e30c3d29714fb4dc34fd1f5eaf05bd86fbba0d -->
...
Required fields: [repo] must contain exactly one of url or path. When [judge] is present, [judge].measure_cmd and [judge].direction are required. When [judge] is absent, the run is a task lane.
...
setup_cmd default when omitted: the engine must git clone [repo] into [workspace].dir and git checkout [ref]. The workspace must end up a git repo.
...
- plan_admitted: { plan_version, reason, budget_usd, tasks }, emitted once after the graph is admitted and before any dispatch. Each task carries its name, kind, dependencies, session, needs, required flag, join, stage, and whether declared failure evidence is captured. This is the consumer's route to the graph, so no gate label has to encode it. A consumer computing a verdict must skip tasks whose stage is epilogue, which is why the field is on the wire rather than inferable. A reader of an event written before the failure-capture field existed must interpret its absence as false.

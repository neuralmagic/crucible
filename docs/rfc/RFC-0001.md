...
<!-- SIGNATURE: sha256:ba884044412160e0ebfba3290b46d8c5107547dacbc58ba218e7962692b79cb3 -->
...
A model-decided route must declare min_confidence in (0, 1]. When the probability the model assigns its chosen label is below it, the engine must record the answer as "uncertain". The task's output must carry, for each question, the resolved label and the full probability distribution the model returned, so the record that resume folds under [RFC-0002:C-PLAYBOOK-RESUME](../rfc/RFC-0002.md#rfc-0002c-playbook-resume) is the decision itself and a resumed run never asks again.

An output-decided route must name a task among its dependencies. That task's output must carry, under each question id, one of the question's declared labels or "uncertain" as a string; a noul also accepts a boolean. A missing field, any other value, or any other type must fail the route. When the named task declares emits, validation must reject a question id the declaration omits. The recorded answer carries probability 1 for the label read, and an empty distribution with confidence 0 for "uncertain". An output-decided route makes no network call, spends nothing, and needs no capability.

The engine must reach the decision model through the broker, at an endpoint and credential the controller configures and the workflow cannot name. A model-decided route task must declare needs = "systemone", verified before dispatch under [RFC-0002:C-PLAYBOOK-CAPS](../rfc/RFC-0002.md#rfc-0002c-playbook-caps). The request and response follow the System One decision API: a state object and a map of question id to type, instructions, and options, answered by a probability for a noul and by per-option probabilities for a choice. A response that omits a question, names an undeclared option, or carries a probability outside [0, 1] must fail the task. Failure to reach the endpoint is a transport failure and retries as one.

A human-decided route must declare a deadline of at least one minute and at most seven days, and must declare needs = "human", verified before dispatch under [RFC-0002:C-PLAYBOOK-CAPS](../rfc/RFC-0002.md#rfc-0002c-playbook-caps). Validation must reject a human-decided route whose questions its channel cannot deliver as one message; for slack that is a question with more than 25 labels, an authored text over 3000 characters once escaped, or a message over 50 blocks with every question open. The engine must reach the person through an elicitation endpoint and credential the controller configures and the workflow cannot name; a workflow cannot name a channel, a user, a message, or a destination URL. The delivered question must carry only compile-time authored text (instructions, labels, and option descriptions), the run and task names, the deadline, and a controller-owned run link; nothing a task emits at run time may reach it. A person answers each question by choosing one control per declared label: the question must offer no free-text input and no control for "uncertain". Only a member of the channel the controller configures may answer, the first accepted answer to a question is final, and the endpoint must reject a click from anyone else or naming anything the stored question does not declare. The engine must fail the route rather than record an answer that names an undeclared question, a label the question does not declare, or "uncertain".

A label a person chose is recorded with probability 1 for that label and confidence 1, as an output-decided answer is. When the deadline passes before a question is answered, the engine must record that question as "uncertain" with an empty distribution and confidence 0, keep the answers already given, and pass the route. The run's wall-clock ceiling still bounds the wait: when it arrives before every question is answered and before the deadline, the route must fail and the run must end as having reached its wall-clock ceiling under [RFC-0002:C-PLAYBOOK-BUDGET](../rfc/RFC-0002.md#rfc-0002c-playbook-budget). A human-decided route spends nothing.

The deadline is fixed when a question is first delivered. The endpoint must key a question by the run and the route task, so that delivering it again, on a transport retry or when a resumed run re-dispatches a route that never settled, returns the stored question's answers and the time left before the original deadline and posts nothing new; a key delivered again with different questions must be refused. Failure to reach the endpoint is a transport failure and retries as one, and the engine must not treat a question it could not read back as expired.
...
### [RFC-0001:C-INFERENCE] Injected inference bindings (Normative) <a id="rfc-0001c-inference"></a>

An orchestrator tells the engine where models are reached through one document, the resolved inference bindings, carried as JSON in the environment variable CRUCIBLE_INFERENCE. The document is defined once, in the contract crate both sides compile, and carries a wire version. The engine must reject a document whose version it does not know or that has an unknown field, before it clones, spends, or writes anything, with an error that names the version or the field. Absent or empty, the run has no bindings.

A binding states what the inference is for and how it is reached: a role, a protocol, a model, optionally a URL, and optionally the name of the environment variable that holds its credential. An absent URL means the protocol's own service address; a decision binding must name one, because system_one has none. The document must never carry a credential's value. The roles are agent, the model an agent turn thinks with, and decision, the model a route task asks under [RFC-0001:C-ROUTE](../rfc/RFC-0001.md#rfc-0001c-route). The protocols are messages, chat_completions, responses, and system_one. A document must hold at most one binding per role. An agent binding must speak messages, chat_completions, or responses; a decision binding must speak system_one. A URL must be absolute http or https, name a host, and carry no credentials. A credential variable name must be a valid environment variable name and must not be CRUCIBLE_INFERENCE. The engine must reject a document that breaks any of these, naming the binding.

A binding says nothing about delivery. The engine derives it from the role's consumer. Where the engine's own process is the client, as for a decision, the engine must use the binding directly and the credential must not leave the engine. Where the client runs inside a sandbox, as an agent harness does, the engine relays: it must project the binding into the sandbox under the names that client reads, and must add the binding's endpoint to the sandbox's egress policy. Those names are the engine's concern and are not part of this contract; an orchestrator must not be required to know them.

The systemone capability of [RFC-0002:C-PLAYBOOK-CAPS](../rfc/RFC-0002.md#rfc-0002c-playbook-caps) is available exactly when the document holds a decision binding. A named credential variable that is unset or empty when its binding is first used must fail that use with an error naming the variable, and must not be treated as an unauthenticated endpoint.

An agent binding is the whole answer for agent turns. Its model is the run's model, over the manifest's agent defaults and over a model the launch named; a task that names its own model keeps it. Its protocol constrains the harness: the harness the launch or the manifest selected is kept when it is a client of that protocol, and otherwise the engine must select the harness that is. Its credential takes the place of any credential the pack's manifest selects, in every authentication mode that manifest names. When an agent binding is present the engine must not consult the process's ambient inference variables. The engine must log each harness, model, or credential a binding replaces. An agent binding with no credential variable authenticates with the deployment's ambient credentials, as a run with no agent binding does. A run with no agent binding reaches its model the way its manifest and the process's ambient environment say, which is how an operator's own shell runs one.

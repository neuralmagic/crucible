...
<!-- SIGNATURE: sha256:557bfec4c48b236b7339b11da3bfbbb14d98e6922277a439535ea5046ca11ade -->
...
A playbook's graph says what runs before what, and what must pass before what runs. The graph is dispatched once, so a task that must retry, poll, or converge does that inside its own execution, where the author's command or the agent's own turn loop owns the repetition. The one repetition the graph states is a revise loop under [RFC-0002:C-REVISE-LOOP](../rfc/RFC-0002.md#rfc-0002c-revise-loop): a reviewer sends a failing verdict back to the single dependency it names, for a number of rounds fixed before any spend. It is declared on an edge that already exists, so it adds no node and no edge, and its bound is visible in the compiled graph. A repair that re-runs more than one producer, or whose bound depends on what a task finds, is still one task, and the engine never sees that iteration at all.
...
A task MUST declare the workflows it may ask for, and the engine MUST reject such a declaration outside the playbook lane, on any task other than an agent, command, or evaluate task, and on either task of a revise loop under [RFC-0002:C-REVISE-LOOP](../rfc/RFC-0002.md#rfc-0002c-revise-loop), whose rounds produce drafts the loop may replace. A task emits asks by returning them in its JSON output under the reserved field "asks", as a list of objects each carrying exactly "key", "workflow", and "params". A task MUST NOT declare that field among its outputs, and the engine MUST reject a graph that does.

Only a task that settles passing emits asks. The engine MUST NOT record, count, or act on the asks in the output of a task that settled any other way. The engine MUST check a passing task's asks before that task's work is committed to the run's git memory, and MUST settle the task as a measured failure, retaining its output and not retrying it, when its output carries asks and the task declares no workflow, when the field is not a list, when an ask has any field besides the three, when an ask's key or workflow name is unusable, when an ask names a workflow the task did not declare, when a parameter value is anything other than a string, a number, a boolean, or a list of strings, when one ask's encoded parameters exceed 4096 bytes, or when the task repeats a workflow and key that it or an earlier task of the same run already emitted. The workflow and key together identify an ask, since two workflows asking about one item are two pieces of work.

The bound counts asks across the whole run. The launcher sets it, the engine MUST apply a default when the launcher sets none, and the admitted-plan event MUST state the bound in force alongside each task's declared workflows. A task whose asks would take the run's total past the bound MUST fail as a measured failure, the engine MUST NOT record or count any of its asks, and a later task MUST see the total as it stood before the refused task. Tasks dispatched together meet the bound in declaration order, so which of them fails does not depend on which finished first.

The engine MUST record a passing task's asks as one event on the session log, written before that task's result row, so a reader that has seen the row has seen what the task asked for.
...
The engine MUST dispatch a task joining "settled" once every one of its dependencies has reached a terminal status, whatever that status is: passed, failed, not taken under [RFC-0001:C-ROUTE](../rfc/RFC-0001.md#rfc-0001c-route), skipped whether the task declared the skip or the substrate did, transport-failed after its retries, or blocked. A settled join MUST NOT make a dependency's runnability a condition of the dependent's; the engine MUST treat such a task as runnable wherever the substrate satisfies its own capability requirement, whatever its dependencies require.
...
### [RFC-0002:C-REVISE-LOOP] A reviewer's bounded send-back (Normative) <a id="rfc-0002c-revise-loop"></a>

A task MAY declare revise, naming one task, together with max_rounds. The named task is the target and the declaring task is its reviewer. The engine MUST reject revise without max_rounds and max_rounds without revise, MUST reject max_rounds outside 2 to 5 inclusive, and MUST reject a target that is not a direct dependency of its reviewer. The engine MUST reject a revise loop in which either task is anything other than an agent, command, or evaluate task, or in which either task maps over a list. The engine MUST reject a target named by two reviewers, a target that is itself a reviewer, and a reviewer that is itself a target: revise loops do not nest or chain. The engine MUST reject a reviewer with any other dependency from which the target is reachable, because that dependency would read a draft a later round replaces. The engine MUST reject revise in every workflow type other than "playbook". The name "revision" is a reserved input under [RFC-0002:C-SETTLED-JOIN](../rfc/RFC-0002.md#rfc-0002c-settled-join), and the engine MUST reject a dependency named after it.

The engine MUST NOT dispatch the loop's first round until the target is ready under its own join and every other dependency of the reviewer has settled, and MUST NOT dispatch a dependent of either task until both have settled under their own names. Where the reviewer cannot run on the substrate, the engine MUST dispatch the target as a task outside any loop and MUST settle the reviewer as [RFC-0002:C-PLAYBOOK-CAPS](../rfc/RFC-0002.md#rfc-0002c-playbook-caps) requires.

A round dispatches the target, then dispatches the reviewer against that round's target result under the reviewer's own join. A reviewer whose join that result does not satisfy MUST settle blocked for that round, as any task would. The engine MUST begin another round when, and only when, the reviewer's round settled failing and fewer than max_rounds rounds have run. A reviewer round that passed, skipped, transport-failed, or was blocked ends the loop, as does a target round whose failure leaves the reviewer blocked. Before every round after the first, and before every reviewer dispatch, the engine MUST check the cost and wall-clock ceilings; an exhausted ceiling ends the loop and halts the run as it would halt any dispatch, and a reviewer not dispatched for it MUST settle blocked on that ceiling.

From the second round on, the engine MUST give the target the reserved input "revision", carrying under "round" the round about to run, under "max_rounds" the declared bound, under "reviewer" the reviewer's name, and under "review" the reviewer's previous round as the entry [RFC-0002:C-SETTLED-JOIN](../rfc/RFC-0002.md#rfc-0002c-settled-join) defines for a settled dependency. Where the engine holds a complete file set captured from that failing reviewer round, it MUST stage that set into the target under the reviewer's name, and the entry's staged-file flag MUST say whether it did. The engine MUST NOT give the target "revision" in the first round. A target that declares a session MUST resume that session in every round.

Each round of each task MUST produce one session row named after the task followed by "[round-N]", written as that round settles and carrying that round's status, output, note, and spend. Once the loop ends, each task MUST produce one row under its own name carrying its last round's status, output, note, blocked reason, and transport cause, and the sum of every round's spend. A round row MUST NOT affect the verdict or halt the run; the rows under the tasks' own names do, exactly as [RFC-0002:C-PLAYBOOK-LANE](../rfc/RFC-0002.md#rfc-0002c-playbook-lane) states for any task. A dependent of either task MUST read the last round's output and captured files.

Every round settles its work as any task does. A passing round's workspace changes MUST be committed to the run's git memory and a failing round's MUST be discarded, as [RFC-0002:C-PLAYBOOK-LANE](../rfc/RFC-0002.md#rfc-0002c-playbook-lane) requires, so a later round runs on top of every earlier passing round. A round's captured file set MUST replace the set its task published in an earlier round under the rules of [RFC-0002:C-TASK-FILES](../rfc/RFC-0002.md#rfc-0002c-task-files) and [RFC-0002:C-SETTLED-JOIN](../rfc/RFC-0002.md#rfc-0002c-settled-join).

The admitted-plan event MUST carry a reviewer's target under "revise" and its bound under "max_rounds", and MUST carry an empty string and zero for every other task.
...
- a settled join forwards a dependency that settled not taken
- C-ASKS states how a task declares and emits asks, the reserved output field, the run bound, and where asks land on the session log

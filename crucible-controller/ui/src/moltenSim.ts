...
const CLASS: Record<string, number> = { ".": 0, W: 1, R: 2, C: 3, M: 4, A: 5 };
const G = 96;
const SCALE = G / 16;
...
export const MASTHEAD_DEFAULTS: MoltenOptions = {
  particles: 8192,
  fps: 30,
  gravity: 70,
  stiffness: 1.5,
...
export async function startMolten(
  canvas: HTMLCanvasElement,
  opts: MoltenOptions
): Promise<MoltenHandle | null> {
  ...
  const pipeOf = (entryPoint: string) =>
  ...
  function simStep(encoder: GPUCommandEncoder, dt: number, kick: number) {
    t += dt;
    pdata.set([
  ...
  function shadeAndDraw(encoder: GPUCommandEncoder) {

...
fn run_turn_with(
    ...
    crate::process::pid_registry::register(child_pid);
...
impl Args {
    ...
    pub(crate) fn agent_source(&self) -> AgentSource {
        if self.agent_backend == crate::manifest::AgentBackend::Command {
            return AgentSource::Command(self.agent_cmd.clone().unwrap_or_default());
        }
        match self.agent_backend {
            crate::manifest::AgentBackend::Openshell => AgentSource::OpenshellDriver,
            // Local (and the Command case handled above) spawn claude directly.
            _ => AgentSource::LocalClaude,
        }
    }
}

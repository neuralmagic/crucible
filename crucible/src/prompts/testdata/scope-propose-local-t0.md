...
**Confirmed tier: T0.** A ranker has already decided which gate shape this issue needs
...
## What to draft into `_pack`

Write into `_pack` — a directory path relative to your current working directory, already
...
1. **`crucible.toml`** with:
   - `[repo]` — copy this line VERBATIM, character for character: `url = "https://x.invalid/x.git"`. Do NOT
...
Either `_pack` holds a complete pack (`crucible.toml` with `[judge.selftest]`, the measure
script, the goal, and any authored `workflow.star` plus prompt files), or
`_pack/REJECTED.md` explains why this issue can't be fairly gated this way. Nothing else is

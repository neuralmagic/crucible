...
/// [`crate::control::recovery::classify_session`] can drive it and the tail scanner in one pass.
...
pub(crate) fn plan_recovery(s: &SessionRecovery, iterations: u32, max_cost: f64) -> RecoveryPlan {
    let finished = resume_finished(&s.resume, iterations, max_cost);
...
pub(crate) fn resume_approval(
    rec: &ResumeRecovery,
    replay: Option<&crate::control::admission::ResumeReplay>,
) -> ResumeApproval {
    ...
    if rec
        .repark
        .as_ref()
        .is_some_and(|pp| pp.handle == crate::control::distress::HANDLE)
    {
...
mod tests {
    ...
    fn dangling_agent_start_classifies_died_mid_turn_with_evidence() {
        ...
        events.push(SessionEvent::AgentSession {
            session: "solver".into(),
            action: crate::report::session::SessionAction::Resumed,
            turn: 3,
        });
        events.push(SessionEvent::AgentStart { iter: 2 });
        ...
        events.push(SessionEvent::Agent {
            event: AgentEvent::Tokens(crate::agent::event::Tokens {
                input: 10,
                output: 5,
                cache_read: 0,
                cache_write: 0,
    ...
    fn replay_holding(
        rescope: Option<(AdmissionKey, &str)>,
    ) -> crate::control::admission::ResumeReplay {
        crate::control::admission::ResumeReplay {
    ...
    fn a_distress_wait_never_reparks() {
        ...
        let rec = ResumeRecovery {
            ...
            repark: Some(provisioning::PendingProvisioning {
                mode: WaitMode::Block,
                trace_id: crate::control::distress::HANDLE.into(),
                handle: crate::control::distress::HANDLE.into(),
            }),
            pending_regime: Some(crate::control::distress::HANDLE.into()),
        };

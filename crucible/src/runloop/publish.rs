...
pub fn fetch_prior_results(results_bucket: &str, goal: &str) -> Option<String> {
    ...
    let md = crate::agent::engine::handle().ok()?.block_on(async {
        let conf = aws_config::load_defaults(aws_config::BehaviorVersion::latest()).await;
        let client = aws_sdk_s3::Client::new(&conf);
...
mod tests {
    ...
    fn pr_body_charts_scores_and_embeds_declared_artifacts() {
        ...
        let mut args = <crate::cli::Cli as clap::Parser>::try_parse_from(["crucible"])
            .unwrap()
            .run;
        ...
        let paths = crate::args::Paths::for_manifest(
            ws.clone(),
            std::env::temp_dir(),
            &std::env::temp_dir(),
            None,
    ...
    fn task_run_pr_body_has_kept_sections_and_no_scores() {
        let args = <crate::cli::Cli as clap::Parser>::try_parse_from(["crucible"])
            .unwrap()
            .run;
        let paths = crate::args::Paths::for_manifest(
            std::env::temp_dir(),
...
",
        ...
        let paths = crate::args::Paths::for_manifest(ws.clone(), base.join("state"), &base, None);
        let rows = vec![Row {
            iter: 1,
            decision: "keep".into(),
            score: Some(8.5),
...
"
    ...
    fn malformed_session_log_publishes_without_flow_files() {
        ...
        let paths = crate::args::Paths::for_manifest(ws, base.join("state"), &base, None);
        let rec = Record {
            args: &args,
            paths: &paths,
            run_id: "20260811T000000Z-goal",
...
| 2 | keep | skip double JSON parse | {} |\n";
    ...
    fn a_refusal_is_recorded_on_the_session_log_and_is_not_an_error() {
        ...
        match crate::report::session::decode(text.trim()).expect("decodes") {
            crate::report::session::SessionEvent::OutputRefused {
                output_kind,
                bound,
                tool,
                ..

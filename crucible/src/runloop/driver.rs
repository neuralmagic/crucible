...
//! workspace prep, front-end choice) lives in [`crate::cli::run`]; this module is just the loop and
//! its helpers.
...
pub(crate) struct LoopRuntime {
    ...
    pub ledger: Option<std::sync::Arc<crate::control::admission::AdmissionLedger>>,
    ...
    pub heartbeat: Option<std::sync::Arc<crate::control::heartbeat::Heartbeat>>,
}
...
pub(crate) fn run_loop<R: Reporter>(
    ...
    let mut runner = crate::runloop::graph::LoopTaskRunner::new(
        args,
        p,
        reporter,
        Arc::clone(world),
    ...
    let outcome = run_loop_body(args, p, prep, &mut runner, world, judge, runtime);
    if let Err(e) = &outcome {
        runner.r.shutdown("error", &format!("{e:#}"));
    }
    (runner.r, outcome)
}
...
fn run_loop_body<R: Reporter>(
    ...
    p: &Paths,
    prep: &Prepared,
    runner: &mut crate::runloop::graph::LoopTaskRunner<R>,
    world: &Arc<dyn World>,
    judge: &Arc<dyn Judge>,
    runtime: LoopRuntime,
) -> Result<Outcome> {
    ...
    let mut preflight_baseline: Option<crate::runloop::preflight::PreflightBaseline> = None;
    ...
    let mut run = if let Some(rs) = runtime.resume {
        ...
        if let Some(why) = &resumed_best.degraded {
            runner.r.note(&format!("resume: {why}"));
        }
        // The ledger is read FIRST: a grant recorded before the death settles what the
        // dangling approval bracket means.
        let replay = ledger.map(crate::control::admission::AdmissionLedger::replay_for_resume);
        if let Some(rec) = runtime.recovery {
            runner.r.recovery(rec.class, rec.iter, &rec.detail);
            let approval = crate::control::recovery::resume_approval(&rec, replay.as_ref());
            if let Some(why) = &approval.note {
                runner.r.note(why);
            }
            if let (Some(control), Some(regime)) = (control.as_ref(), approval.pending_regime) {
                control.set_pending_regime(regime);
            }
            run.pending_block = approval.repark;
        }
        if let (Some(ledger), Some(replay)) = (ledger, replay) {
            replay_admissions(ledger, control.as_deref(), replay, &mut runner.r);
        }
        ...
        runner.r.identity(&prep.identity);
        ...
        {
            machine.advance(Event::PreflightStarted, &mut runner.r)?;
            update_control_progress(
                control.as_deref(),
                start_iter.saturating_sub(1),
                run.segment.best_score,
            ...
            match preflight {
                Ok(seeded) => {
                    machine.advance(Event::PreflightPassed, &mut runner.r)?;
                ...
                Err(e)
                    if e.downcast_ref::<crate::runloop::preflight::PreflightStopped>()
                        .is_some() =>
                {
                    machine.advance(Event::Stopped, &mut runner.r)?;
        ...
        write_results(p, &prep.goal, &prep.prior, &run.rows)?;
        machine.advance(Event::Resumed, &mut runner.r)?;
        run
    } else {
        ...
        if machine.state() == State::Setup {
            machine.advance(Event::BaselineStarted, &mut runner.r)?;
        }
        update_control_progress(control.as_deref(), 0, f64::INFINITY, 0.0);
        ...
        machine.advance(Event::BaselineMeasured, &mut runner.r)?;
        run
    };
    ...
    if !is_resume
        && let Some(wide_cfg) =
            crate::runloop::graph::WideConfig::resolve(args, args.search.as_ref())
    {
        ...
        machine.advance(Event::WideStarted, &mut runner.r)?;
        ...
        if let Some(winner_id) = winner {
            ...
            if let Err(e) = applied {
                ...
                match world.snapshot("wide: winner applied") {
                    ...
                    Err(e) => runner
                        .r
                        .note(&format!("snapshot after wide winner failed: {e:#}")),
                }
            }
        } else {
            runner
        ...
        machine.advance(Event::WideDone, &mut runner.r)?;
    }
    ...
    while it <= args.iterations {
        machine.set_iter(it, &mut runner.r);
        if control.as_deref().is_some_and(|c| c.is_paused()) {
            machine.advance(Event::Paused, &mut runner.r)?;
            wait_if_paused(control.as_deref(), &mut runner.r);
            machine.advance(Event::Unpaused, &mut runner.r)?;
        }
        ...
        if let Some(pp) = run.pending_block.take() {
            machine.advance(Event::ApprovalPending, &mut runner.r)?;
            let parked = park_for_approval(
                control.as_deref(),
                ledger,
                &mut runner.r,
                &mut run.parked_total,
                args.max_park(),
            );
            match parked {
                // The re-scope drain below re-baselines.
                ParkOutcome::Resumed => {
                    machine.advance(Event::ApprovalGranted, &mut runner.r)?;
                }
                ParkOutcome::Denied(why) => {
                    ...
                    machine.advance(Event::ApprovalDenied, &mut runner.r)?;
                    break;
                }
                ParkOutcome::Stopped => {
                    machine.advance(Event::Stopped, &mut runner.r)?;
        ...
        if let Some((rescope_key, new_regime)) = control.as_ref().and_then(|c| c.take_rescope()) {
            machine.advance(Event::Rescoped, &mut runner.r)?;
        ...
        if let Some((deny_key, reason)) = control.as_ref().and_then(|c| c.take_deny()) {
            runner.r.approval_resolved("denied", &reason);
            runner.r.note(&format!(
        ...
        match crate::control::distress::read_marker() {
            Some(Ok(marker)) if parked_distress_ts != Some(marker.ts_ms) => {
                ...
                machine.advance(Event::DistressMarked, &mut runner.r)?;
                ...
                match parked {
                    DistressOutcome::Cleared => {
                        machine.advance(Event::DistressCleared, &mut runner.r)?;
                        runner
                            .r
                            .approval_resolved("granted", "distress cleared by operator");
                        runner.r.note("distress cleared, resuming");
                    ...
                    DistressOutcome::TimedOut => {
                        ...
                        machine.advance(Event::ParkTimedOut, &mut runner.r)?;
                        break;
                    }
                }
            }
            ...
            Some(Err(why)) => {
                let mtime = crate::control::distress::marker_mtime();
                if bad_marker_seen != Some(mtime) {
                    bad_marker_seen = Some(mtime);
                    runner
                        .r
                        .note(&format!("distress marker unreadable ({why}), not parking"));
                }
            }
            _ => {}
        }
        if matches!(runner.r.check_interrupt(p, &run.rows), Stop::Quit) {
            machine.advance(Event::Interrupted, &mut runner.r)?;
            break;
        }
        if over_budget(
            args,
            ...
            machine.advance(Event::OverBudget, &mut runner.r)?;
            break;
        }
        machine.advance(Event::TurnStarted, &mut runner.r)?;
        ...
        let steer_batch = crate::control::admission::drain_steer(ledger, &p.steer);
        if steer_batch.text.is_some() {
            runner.r.note("injected operator steer");
        }
        ...
        let Decided {
            ...
            IterStep::Discarded { reason } => {
                machine.advance(Event::Discarded, &mut runner.r)?;
                let mut row = Row {
                    iter: it,
                ...
                fold_distress_notes(&mut runner.r, &mut row.note);
                runner.r.row(&row, false);
                run.rows.push(row);
                world.restore(run.segment.best_snap.as_str())?;
                it += 1;
            ...
            IterStep::NeverStarted { reason } => {
                ...
                if dead_turns >= MAX_DEAD_TURN_ATTEMPTS {
                    ...
                    machine.advance(Event::Stalled, &mut runner.r)?;
                    break;
                }
                runner.r.note(&format!(
                    "turn never started (attempt {dead_turns}/{MAX_DEAD_TURN_ATTEMPTS}) — re-running iter {it} without consuming it"
                ));
                machine.advance(Event::TurnNeverStarted, &mut runner.r)?;
                continue;
            }
            IterStep::Escalated => {
                update_control_progress(control.as_deref(), it, run.segment.best_score, run.spent);
                world.restore(run.segment.best_snap.as_str())?;
                machine.advance(Event::Escalated, &mut runner.r)?;
                break;
            }
            IterStep::Parked(pp) => {
                machine.advance(Event::Parked, &mut runner.r)?;
                update_control_progress(control.as_deref(), it, run.segment.best_score, run.spent);
        ...
        if verdict.keep {
            ...
            match world.snapshot(&format!("iter {it}: keep ({})", reading.note)) {
                ...
                Err(e) => runner
                    .r
                    .note(&format!("snapshot failed (change still live): {e:#}")),
            }
        ...
        runner.r.row(&row, verdict.solved);
        run.rows.push(row);
        write_results(p, &prep.goal, &prep.prior, &run.rows)?;
        ...
        if verdict.keep && verdict.solved && !args.no_early_stop {
            machine.advance(Event::Solved, &mut runner.r)?;
            break;
        }
        it += 1;
    }
    if machine.state() == State::Head {
        machine.advance(Event::IterationsExhausted, &mut runner.r)?;
    }
    ...
    {
        machine.advance(Event::EpilogueStarted, &mut runner.r)?;
        let kept = run
            .rows
            .iter()
            .rev()
            .find(|row| row.decision == "keep")
            .map(|row| crate::runloop::graph::KeptContext {
                iter: row.iter,
                score: row.score,
        ...
        match kept {
            None => runner.r.note("epilogue skipped: the run kept nothing"),
            Some(kept) => {
                ...
                if let Err(e) = world.restore(run.segment.best_snap.as_str()) {
                    ...
                    let epilogue = crate::runloop::graph::run_epilogue(
                        args,
                        p,
                        workflow,
                        &kept,
                    ...
                    match epilogue {
                        ...
                        Err(e) => runner
                            .r
                            .note(&format!("epilogue failed to run (advisory): {e:#}")),
                    }
                }
            }
        }
        machine.advance(Event::EpilogueDone, &mut runner.r)?;
    }
    ...
    let components = run
        ...
        .unwrap_or_default();
    machine.advance(Event::PublishStarted, &mut runner.r)?;
...
fn restore_kept_best(
    world: &dyn World,
    direction: crucible::crucible::Direction,
    rows: &[Row],
    logged_best: f64,
...
fn beat_position(heartbeat: Option<&crate::control::heartbeat::Heartbeat>, iter: u32, spent: f64) {
...
fn fold_distress_notes<R: Reporter>(r: &mut R, note: &mut String) {
    for (severity, reason) in crate::control::distress::drain_notes() {
...
fn park_for_distress<R: Reporter>(
    ...
    let start = Instant::now();
    let outcome = loop {
        if crate::control::distress::read_marker().is_none() {
...
enum BaselineSource {
    ...
    Preseeded(crate::runloop::preflight::PreflightBaseline),
}
...
fn baseline_source(
    skip: bool,
    preflight: Option<&crate::runloop::preflight::PreflightBaseline>,
) -> BaselineSource {
    match preflight {
...
fn worst_score(direction: crucible::crucible::Direction) -> f64 {
    match direction {
        crucible::crucible::Direction::Higher => f64::NEG_INFINITY,
        crucible::crucible::Direction::Lower => f64::INFINITY,
    }
}
...
mod tests {
    ...
    fn load_resume_state(session_log: &std::path::Path) -> Result<ResumeState> {
        crate::control::recovery::classify_session(session_log).map(|s| s.resume)
    }
    ...
    fn resume_restores_kept_tree_and_rebuilds_keeps() {
        ...
        let best = restore_kept_best(
            &world,
            crucible::crucible::Direction::Lower,
            &rows,
            180.0,
            Some(0.5),
        )
    ...
    fn resume_without_restorable_snap_resets_best() {
        ...
        let best = restore_kept_best(
            &world,
            crucible::crucible::Direction::Higher,
            &rows,
            42.0,
            Some(0.5),
        )
    ...
    impl Reporter for NoteCapture {
        ...
        fn phase(&mut self, _: LoopPhase, _: u32) {}
    ...
    fn distress_fixture(
        name: &str,
        iterations: u32,
    ) -> (Fixture, crate::control::distress::testing::Root) {
        let f = fixture(iterations, 0.0, false);
        let root = crate::control::distress::testing::Root::new(name);
        (f, root)
    }
    ...
    fn a_marker_parks_the_head_and_the_operator_rm_resumes_the_run() {
        ...
        outcome.expect("a granted distress resumes, it does not error");
        ...
        assert_eq!(
            ...
            vec![
                ...
                (LoopPhase::Iteration, 2),
                (LoopPhase::Finished, 2),
            ],
            "the session log reads parked while the park happens"
        );
    ...
    fn a_malformed_marker_is_noted_once_and_never_parks() {
        ...
        outcome.expect("a bad byte must never wedge a paid run");
    ...
    fn a_marker_surviving_a_restart_reparks_without_a_second_row() {
        ...
        outcome.expect("the park resolves when the operator clears it");
        operator.join().unwrap();
        assert!(
            r.rows.iter().all(|row| row.decision != "distressed"),
    ...
    impl Judge for FakeJudge {
        ...
        fn direction(&self) -> crucible::crucible::Direction {
            crucible::crucible::Direction::Lower
        }
    ...
    struct RecordingReporter {
        ...
        recoveries: Vec<(crate::report::session::RecoveryClass, u32, String)>,
        waits: Vec<(String, String, provisioning::WaitMode)>,
        resolved: Vec<(String, String)>,
    ...
    impl Reporter for RecordingReporter {
        ...
        fn recovery(
            &mut self,
            class: crate::report::session::RecoveryClass,
            iter: u32,
            detail: &str,
        ) {
    ...
    fn seed_diff_reaches_iter_one_prompt_only() {
        ...
        outcome.expect("run_loop should finish cleanly");
    ...
    fn shutdown_emitted_once_on_a_clean_finish() {
        ...
        let outcome = outcome.expect("run_loop should finish cleanly");
    ...
    fn shutdown_emitted_once_on_solved() {
        ...
        let outcome = outcome.expect("run_loop should stop early on the solve");
        assert!(outcome.solved);
        assert_eq!(
            r.shutdowns.len(),
            1,
    ...
    fn shutdown_emitted_once_on_stop() {
        ...
        outcome.expect("a stop signal is a clean exit, not an error");
        assert_eq!(
            r.shutdowns.len(),
            1,
            "exactly one shutdown call: {:?}",
    ...
    fn epilogue_workflow(command_toml: &str) -> crate::plan::workflow::WorkflowCfg {
        let workflow: crate::plan::workflow::WorkflowCfg = toml::from_str(&format!(
            "[[task]]\nname = \"racecheck\"\nkind = \"command\"\n{command_toml}\nstage = \"epilogue\"\n"
        ))
    ...
    fn epilogue_failure_is_advisory_and_does_not_unkeep() {
        ...
        outcome.expect("an epilogue failure must not fail the run");
    ...
    fn task_lane_keeps_every_turn_unscored_and_finishes() {
        ...
        let outcome = outcome.expect("a task run finishes");
    ...
    fn shutdown_emitted_once_on_budget() {
        ...
        outcome.expect("a budget stop is a clean exit, not an error");
        assert_eq!(
            r.shutdowns.len(),
            1,
            "exactly one shutdown call: {:?}",
    ...
    fn shutdown_emitted_once_on_escalation() {
        ...
        let outcome =
            outcome.expect("an escalation is a clean exit (its own exit code), not an error");
    ...
    fn is_error_turn_is_discarded_before_measuring() {
        ...
        let outcome =
            outcome.expect("a failed agent turn is a discarded iteration, not a run error");
        assert!(
            !outcome.solved,
    ...
    fn started_turn_resets_the_dead_streak() {
        ...
        outcome.expect("the streak resets, so the run finishes");
    ...
    fn shutdown_once_on_a_clean_finish() {
        ...
        let outcome = outcome.expect("graph loop should finish cleanly");
        assert!(!outcome.solved);
        assert_eq!(
            r.shutdowns,
            vec![("finished".into(), "all iterations completed".into())]
    ...
    fn stops_early_on_solved() {
        ...
        let outcome = outcome.expect("graph loop should stop early on the solve");
        assert!(outcome.solved);
        assert_eq!(r.shutdowns.len(), 1, "one shutdown: {:?}", r.shutdowns);
        assert_eq!(r.shutdowns[0].0, "solved");
    }
    ...
    fn discards_an_is_error_turn_before_measuring() {
        ...
        let outcome = outcome.expect("a failed turn is a discarded iteration, not a run error");
    ...
    fn escalation_halts_for_review() {
        ...
        let outcome = outcome.expect("an escalation is a clean exit, not an error");
        assert!(outcome.escalated);
        assert_eq!(r.shutdowns.len(), 1, "one shutdown: {:?}", r.shutdowns);
        assert_eq!(r.shutdowns[0].0, "escalated");
    }
    ...
    fn shutdown_emitted_once_on_error() {
        ...
        let err = outcome.expect_err("a failed baseline must propagate as Err");
        assert!(
    ...
    fn resume_emits_recovery_and_reparks_a_block_approval() {
        ...
        let recovery = crate::control::recovery::ResumeRecovery {
            class: crate::report::session::RecoveryClass::DiedAwaitingApproval,
            iter: 0,
        ...
        outcome.expect("resumed run finishes");
        assert_eq!(r.recoveries.len(), 1, "exactly one recovery line");
        let (class, iter, detail) = &r.recoveries[0];
        assert!(matches!(
            class,
            crate::report::session::RecoveryClass::DiedAwaitingApproval
        ));
        assert_eq!(*iter, 0);
    ...
    fn fixture_ledger(f: &Fixture) -> std::sync::Arc<crate::control::admission::AdmissionLedger> {
        std::sync::Arc::new(
            crate::control::admission::AdmissionLedger::open(
                &f.paths.admissions,
    ...
    fn preflight_failure_refuses_to_start_before_any_agent_turn() {
        ...
        let err = outcome.expect_err("a failing rung is an environment verdict");
        assert!(
    ...
    fn preflight_baseline_seeds_the_segment_and_iteration_one_decides_against_it() {
        ...
        outcome.expect_err("the iteration's own measure still explodes; the baseline did not");
        let base = r
            .rows
            .iter()
            .find(|row| row.iter == 0)
    ...
    fn preseeded_baseline_survives_a_rescope_rebaseline() {
        ...
        let (r, outcome) = run_loop(&f.args, &f.paths, &f.prepared, r, &world, &judge, runtime);
        outcome.expect("the rescope re-baselines cleanly");
        assert!(
    ...
    fn results_md_detail_carries_the_evidence_line() {
        ...
        let p = crate::args::Paths::for_worktree(dir.clone(), None);
        let rows = vec![
            Row {
                iter: 1,
                decision: "keep".into(),
    ...
    /// restore, [`resume_finished`] no-ops a resume of the finished run (the f19d64a rep(&r)), and
    ...
    fn run6_fixture_restores_noops_and_dedupes_publish() {
        ...
        // The run declared 6 iterations and burned them all: resuming is a no-op. The rep(&r)
        ...
        .iter()
        .map(|run_id| crate::runloop::publish::head_branch_for_test(goal, run_id, &kept_shas))
        .collect();
        assert_eq!(
            branches.len(),
            1,
    ...
    fn resume_of_preflight_failed_session_reruns_preflight_and_seeds_baseline() {
        ...
        outcome.expect("resumed run finishes after preflight re-run");
        assert!(
            sentinel.exists(),

//! The flow model and its extraction from a session log, plus the dot and mermaid emitters.
...
pub(crate) fn emit_dot(m: &FlowModel) -> String {
...
pub(crate) fn emit_mermaid(m: &FlowModel) -> String {
    let mut s = format!(
        "---\ntitle: \"{}\"\n---\nflowchart LR\n",
...
mod tests {
    use super::*;
    use crate::flow::{FlowFormat, FlowInput, render};
    use std::path::Path;

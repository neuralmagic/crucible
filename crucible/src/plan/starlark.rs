...
use crate::plan::diag;
use crate::plan::ir::{
    EngineOp, Isolation, Join, MAX_FANOUT_CEILING, MAX_ROUNDS_CEILING, OutputField, OutputRef,
    ReportDestination, Revise, SlackDestination, Stage, Task, TaskKind, TaskName,
};
...
fn dsl_task(
    ...
    if let Some(reserved) = task
        .depends_on
        .iter()
        .find(|d| crate::plan::ir::RESERVED_INPUTS.contains(&d.0.as_str()))
    {
...
"#;
        ...
        let loaded: crate::plan::workflow::WorkflowCfg = toml::from_str::<toml::Value>(&manifest)
            .unwrap()
            .get("workflow")
            .cloned()
            .unwrap()

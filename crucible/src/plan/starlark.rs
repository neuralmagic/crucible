...
fn dsl_task(
    ...
    let task = Task {
        ...
        capture_on_failure: take_bool_default(named, "capture_on_failure", false)?,
...
"#;
    ...
    fn every_declared_kwarg_compiles_and_an_unlisted_one_errors() {
        ...
        let cases: &[(&str, &str)] = &[
            ...
            (
                "command",
                "u = command(name = \"u\", run = \"true\", emits = [\"items\"])\nc = command(name = \"c\", run = \"true\", emits = [\"score\"], emits_files = [\"out.txt\"], capture_on_failure = True, depends_on = [u], needs = \"any\", required = True, isolated = True, join = \"all\", stage = \"iteration\"{extra})\nworkflow(type = \"custom\", tasks = [u, c], result = c)\n",
            ),
            (
                "command",
                "u = command(name = \"u\", run = \"true\", emits = [\"items\"])\nc = command(name = \"c\", run = \"true\", depends_on = [u], over = u.items, max_fanout = 4{extra})\nworkflow(type = \"custom\", tasks = [u, c], result = c)\n",
            ),
            (
                "evaluate",
                "u = command(name = \"u\", run = \"true\", emits = [\"items\"])\ne = evaluate(name = \"e\", run = \"true\", threshold = 1, direction = \"higher\", emits = [\"score\"], emits_files = [\"out.txt\"], capture_on_failure = True, depends_on = [u], needs = \"any\", required = True, isolated = True, join = \"all\", stage = \"iteration\"{extra})\nworkflow(type = \"custom\", tasks = [u, e], result = e)\n",
            ),
            (
                "evaluate",
                "u = command(name = \"u\", run = \"true\", emits = [\"items\"])\ne = evaluate(name = \"e\", run = \"true\", depends_on = [u], over = u.items, max_fanout = 4{extra})\nworkflow(type = \"custom\", tasks = [u, e], result = e)\n",
            ),
            (
                "skill",

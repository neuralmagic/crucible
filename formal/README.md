...
rounds, truncation, halts, and epilogues, with the RFC-0002 playbook obligations as invariants. `#model_check` explores every
well-formed three-task graph and every run of it; `#check_invariants` proves the same invariants
inductive for graphs of any size. Fan-out and transport retries are not modelled yet.

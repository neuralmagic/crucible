...
//! `catch_unwind` only intercepts panics under `panic = "unwind"`; a profile
//! built with `panic = "abort"` gets no in-process isolation, so callers that
//! render untrusted diagrams under that profile should do so in a short-lived
//! child process with a wall-clock timeout. (Upstream ships those subprocess
//! building blocks; this vendored copy keeps only the in-process engine.)
//!
//! # Example
//!
//! ```
...
///
/// `mmdc` is never selected automatically — upstream's `MmdcEngine` opt-in
/// to opt in.
